...
def main():
    ...
    uploaded_distance = st.file_uploader("📂 上傳距離矩陣 (.tsv / .csv)", type=["tsv", "csv"])
    uploaded_metadata = st.file_uploader("📂 上傳 metadata (.xlsx / .csv)", type=["xlsx", "csv"])

    if (not uploaded_distance) or (not uploaded_metadata):
        st.info("📥 請依序上傳距離矩陣與 metadata 檔案")
        return

    try:
        pipeline(distance_file=uploaded_distance, metadat_file=uploaded_metadata)
    except Exception as e:
        st.error(f"❗ 發生錯誤：{e}")


def pipeline(distance_file, metadat_file):
    ...
    if metadat_file.name.endswith(".xlsx"):
        df_meta = pd.read_excel(metadat_file, engine="openpyxl", dtype=str)
    else:
        df_meta = pd.read_csv(metadat_file, dtype=str)
    ...
    if view_mode == "2D":
        ...
        if plot_kind == "categorical":
            sns.scatterplot(x=x_vals, y=y_vals, hue=df_merged[color_var],
                            palette=palette, s=60, edgecolor="black", ax=ax)
            ax.legend(title=color_var, bbox_to_anchor=(1.05, 1), loc='upper left')
        else:
            sc = ax.scatter(x_vals, y_vals, c=df_merged[color_var],
        ...
        st.download_button("📎 下載 2D 圖檔 (PNG, 1200 dpi)", data=buf.getvalue(),
                           file_name=f"{color_var}_PCoA.png", mime="image/png")

    elif view_mode == "3D":
        if all(pc in df_merged.columns for pc in ["PC1", "PC2", "PC3"]):
            try:
                fig3d = px.scatter_3d(df_merged, x="PC1", y="PC2", z="PC3",
                                      color=color_var,
                                      title=chart_title,
                ...
                for fmt in ["png", "pdf", "svg"]:
                    ...
                    st.download_button(f"📎 下載 3D 圖檔 ({fmt.upper()})",
                                       data=image_bytes,
